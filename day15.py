L,R,D=[[*map(int,l[:-1])]for l in open("inputday15")],range,len
def i(L,v,e):
 i=0;v[0]=e[0]+e[1]
...
print(*(r([[[(x:=i+j)and float('inf'),x and(L[i][j]+n+N-1)%9+1,(i+n*D(L),j+N*D(L[0]))]for N in R(l)for j in R(D(L[0]))]for n in R(l)for i in R(D(L))])for l in(1,5)))
